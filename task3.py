...
df_with_watermark = parsed_df.withColumn("timestamp", col("timestamp").cast(TimestampType())).withWatermark("timestamp", "10 minutes")

# Perform windowed aggregation: sum of fare_amount over a 5-minute window sliding by 1 minute
windowed_df = df_with_watermark.groupBy(window(col("timestamp"), "5 minutes", "1 minute")).agg(
...
def write_batch_to_csv(batch_df, batch_id):
    ...
    batch_df.coalesce(1).write.mode("append").csv(f"output/aggregated_{batch_id}", header=True)

# Use foreachBatch to apply the function to each micro-batch
query = aggregated_df.writeStream.foreachBatch(write_batch_to_csv).start()
